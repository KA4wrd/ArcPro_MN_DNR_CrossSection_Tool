...
FieldExists(wwpt_file_orig, "wellname")
...
fields_base = [[wwpt_wellid_field, well_id_field_type], [xsln_etid_field, 'TEXT', '', 3], [wwpt_label_field, 'TEXT', '', 50],
...
fields_2d_point = [['wellname','TEXT'],['dem', 'FLOAT'],['BUFF_DIST','DOUBLE'],['VE','DOUBLE'],['Data_Source','TEXT']]
...
wwpt_fields = [["NEAR_FID", "LONG"], ["NEAR_DIST", "DOUBLE"], ["NEAR_X", "DOUBLE"],
               ["NEAR_Y", "DOUBLE"], ["OnLine_DIST", "FLOAT"], ["dpl_elev", "DOUBLE"],["VE","DOUBLE"]]
...
with arcpy.da.SearchCursor(wwpt_merge, ['OID@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field,'wellname',
                                        'NEAR_DIST', 'SHAPE@X', 'SHAPE@Y', 'OnLine_DIST','dem','BUFF_DIST','VE','Data_Source']) as wwpt:
    for well in wwpt:
        index = well[0]
        wellid = well[1]
        et_id = well[2]
        ...
        dist = well[5] #NEAR_DIST field
        pct_dist = dist / buffer_dist * 200 #percent distance
        real_x = well[6] #true x coordinate of well
        real_y = well[7] #true y coordinate of well
        real_z = well[9]
        if real_z == None:
            printit("Error: Well number {0} is null in elevation field. Skipping.".format(wellid))
            continue
        x_coord_meters = well[8]
        ...
        with arcpy.da.InsertCursor(pointfile, ['SHAPE@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field, 'wellname','x_coord',
                                               'y_coord', 'x_coord_2d','y_coord_2d','distance', 'dem', 'pct_dist','BUFF_DIST','VE','Data_Source']) as cursor:
            # Create geometry and fill in field values, saving true coordinates in attribute
            cursor.insertRow([point_geometry, wellid, et_id, label, wellname, real_x, real_y, x_coord_2d, y_coord_2d, dist, real_z, pct_dist, buff_dist, vert_ex, data_source])
