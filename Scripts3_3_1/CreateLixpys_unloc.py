...
if strat_count == 0:
    printerror("Error: strat table is empty.")
    raise SystemExit
...
with arcpy.da.SearchCursor(strat_table, ['OID@', strat_wellid_field, 'elev_top',
    ...
    for row in strat_records:
        ...
        with arcpy.da.InsertCursor(polylinefile_2d, ['SHAPE@', strat_wellid_field, xsln_etid_field, 'x_coord',
                                                     'y_coord','z_top', 'z_bot', 'strat_oid', 'distance', 'pct_dist','BUFF_DIST','VE']) as cursor2d:
            ...
            cursor2d.insertRow([polyline_geometry, wellid, et_id, real_x, real_y,
                                real_z_top, real_z_bot, strat_oid, dist, pct_dist, buffer_distance,vert_ex])
...
#%% 19 Join strat fields to 2d and 3d polyline feature classes
printit("Joining relevant strat fields to 2d polyline file.")
arcpy.management.JoinField(polylinefile_2d, 'strat_oid', strat_table, 'OBJECTID')
...
polygonfile_copy = arcpy.management.DeleteField(polygon_file, ['strat_oid','x_coord','y_coord', 'distance','pct_dist', 'z_top', 'z_bot',
                                                        'c5st_seq_no','relateid_1','county_c', 'xsec_id_1','ELEVATION','BUFF_DIST','xsec_desc','dem','ORIG_FID','ORIG_FID_1'])[0]

# # make copy of unprojected lixpy poly file
printit("Copying 2D polys.")
polygon_file_copy1 = os.path.join(workspace, "lixpy_unloc_2d_poly_prj")
arcpy.management.CopyFeatures(polygon_file, polygon_file_copy1)
