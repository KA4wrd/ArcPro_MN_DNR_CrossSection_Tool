...
if (len(sys.argv) > 1):
    ...
    xsln = arcpy.GetParameterAsText(1)   #mapview xsln file
    xsec_id_field = arcpy.GetParameterAsText(2)  #et_id in xsln
    intersect_fc = arcpy.GetParameterAsText(3)  #polygon or line file to intersect with xsln. county boundary, faults, papg, etc.
    vertical_exaggeration = int(arcpy.GetParameterAsText(4))
    output_line_fc = arcpy.GetParameterAsText(5)
    spatialref_2d = arcpy.GetParameterAsText(6) # .prj file for custom coordinate system for 2D display
...
# # make copy of unprojected line file
printit("Copying output_line_fc.")
arcpy.env.overwriteOutput = True
line_file_copy1 = os.path.join(workspace, output_line_fc +"_prj")
arcpy.management.CopyFeatures(output_line_fc, line_file_copy1)

# #%% Defining 2D coordinate system for output feature class
arcpy.env.overwriteOutput = True
arcpy.management.DefineProjection(line_file_copy1, spatialref_2d)
