...
fields_2d_point = [['dem', 'FLOAT'],['BUFF_DIST','DOUBLE'],['VE','DOUBLE'],['Data_Source','TEXT']]
...
with arcpy.da.SearchCursor(wwpt_merge, ['OID@', wwpt_wellid_field, wwpt_etid_field,
                                        'NEAR_DIST', 'SHAPE@X', 'SHAPE@Y', 'OnLine_DIST','dem','dropp_len','dpl_elev','BUFF_DIST','VE','Data_Source']) as wwpt:
    for well in wwpt:
        index = well[0]
        wellid = well[1]
        et_id = well[2]
        ...
        if dpl_meas == None:
            printit("Error: Well number {0} is null in ""dropp_len"" field. Skipping.".format(wellid))
            continue
        dpl_z = well_z - dpl_meas #dpl elevation
        if dpl_z == None:
            printit("Error: Well number {0} is null in ""dpl_elev"" field. Skipping.".format(wellid))
            continue
        ...
        with arcpy.da.InsertCursor(pointfile, ['SHAPE@', wwpt_wellid_field, wwpt_etid_field,'x_coord',
                                               'y_coord', 'x_coord_2d','y_coord_2d','distance', 'dem', 'pct_dist', 'dropp_len','dpl_elev','BUFF_DIST','VE','Data_Source']) as cursor:
            # Create geometry and fill in field values, saving true coordinates in attribute
            cursor.insertRow([point_geometry, wellid, et_id, real_x, real_y, x_coord_2d, y_coord_2d, dist, well_z, pct_dist, dpl_meas, dpl_z, buff_dist, vert_ex, data_source])
...
printit('Create 2D DPL Points tool completed at {0}. Elapsed time: {1}. You did it!'.format(toolend, toolelapsed))
